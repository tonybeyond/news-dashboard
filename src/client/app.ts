...
const els = {
  ...
  feed: $("feed"),
  gFilter: $<HTMLInputElement>("g-filter"),
  mFilter: $<HTMLInputElement>("m-filter"),
  gVal: $("g-val"),
  mVal: $("m-val"),
...
function colorFor(g: number): string {
  if (g <= -6) return "#ff5a4d";
  if (g <= -4) return "#ffb648";
  return "#7a8893";
}
function radiusFor(e: GdeltEvent): number {
...
function eventRowHTML(e: GdeltEvent): string {
...
function renderFeed(newIds: Set<string>): void {
  ...
  els.feed.querySelectorAll<HTMLElement>(".event").forEach((el) => {
